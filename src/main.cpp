...
int main(int argc, char* argv[])
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        for (int i = 1; i <= 4; ++i){
            ...
            if (i == 1){
                // Personagem
                model = Matrix_Identity()*Matrix_Translate(0.0f,-1.0f,0.0f)*Matrix_Scale(20.0f,0.0f,3.0f);
...
// Função que desenha um objeto armazenado em g_VirtualScene. Veja deprocessosfinição
